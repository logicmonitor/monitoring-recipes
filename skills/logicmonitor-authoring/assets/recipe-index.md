...
| HTTP REST | `recipes/groovy/http-rest/` | lm.cache, lm.emit (HTTP via `URL.openConnection()`) | DataSource, PropertySource | DS: `key=value` · PS: `auto.prop=value` | Ready |
| Script Logs | `recipes/groovy/script-logs/` | lm.cache (HTTP via `URL.openConnection()`) | LogSource | JSON `{events:[{message}]}` | Ready |
| Script Events | `recipes/groovy/script-events/` | none (HTTP via `URL.openConnection()`) | EventSource | JSON `{events:[{happenedOn,severity,message}]}` | Ready |
